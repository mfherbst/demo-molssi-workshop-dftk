...
# ╔═╡ 1cdcbd7e-8f8b-11ef-27f4-951879c89577
begin
	using PlutoUI
	using LazyArtifacts
	using LinearAlgebra
...
# ╔═╡ 03abd735-f33f-4cf3-b67d-f232407c15fa
md"""
# 1. Custom SCF algorithms
"""

# ╔═╡ 43a3cada-a9c2-41ad-8af1-4b8d979265df
setup_threading(n_blas=1)

# ╔═╡ c718b2b5-4cfe-40a3-80bb-8baa7ae26173
...
# ╔═╡ a97d7ca1-0e0c-4fcd-bb1d-720e3db05439
silicon = attach_psp(bulk(:Si); Si=artifact"pd_nc_sr_pbe_standard_0.4.1_upf/Si.upf");

# ╔═╡ 8307fba7-0621-475c-af1f-f6a1999030f2
md"`damping = ` $(@bind damping Slider(0.1:0.1:1.0; default=0.8, show_value=true))"

# ╔═╡ 4ad0187f-f4d5-4c3e-bb00-7299126c9d66
md"""
## Custom SCF solvers

Let us first demonstrate the ease with which a custom SCF solver is tested in DFTK.
We want to implement the simple damped fixed-point iterations
```math
	\rho_{n+1} = \rho_n + \text{\texttt{damping}} \cdot \Big[ \rho \Big( V(\rho_n) \Big) - \rho_n \Big].
```
where `damping =` $(damping). DFTK directly exposes the function
```math
    F(ρ_n) = \rho\Big( V(ρ_n) \Big)
```
so all we have to do is write the iterative loop around it:
"""

# ╔═╡ f719de61-ce37-4d77-8b60-d9d18e0e81af
function my_damped_mixing(F, ρ₀, maxiter; tol)
...
# ╔═╡ f7a7f8e7-0348-4c7c-85c6-0207fec1954c
begin
	model = model_PBE(silicon)
	basis = PlaneWaveBasis(model; Ecut=10, kgrid=(2, 2, 2), architecture=DFTK.CPU())
	scfres = self_consistent_field(basis;
	                               tol=1e-4,
	                               solver=my_damped_mixing,
	                               damping=1.0)  # Turn off DFTK-internal damping
end;

# ╔═╡ 7b1ec357-8978-4672-a04b-c3fb4db71886
plot(scfres.history_Δρ, yaxis=:log, mark=:x, lw=2, label="Change of ρ")

# ╔═╡ 9ef90555-ed1c-4a2b-8775-287128ce43ce
md"""
Crucially, this integrates **with all HPC features of the code**.
For example we could play with the computational architecture, by changing in the above code
```julia
using CUDA
...
This work because Julia is able to compile high-level functions like `my_damped_mixing` to GPU kernels. GPU coding becomes seamless.
"""
...
manifest_format = "2.0"
project_hash = "97b82e2cf072bafbf1d9c750f6876397bbb5f2fb"
