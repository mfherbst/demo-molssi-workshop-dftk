...
# ╔═╡ 497ccb62-4487-4f23-8a0f-97e5548bda2a
begin
	using PlutoUI
	using DFTK
	using Plots
...
# ╔═╡ be7998d6-8f8c-11ef-3da9-6d4d606e7d51
md"""
# Gross-Pitaevskii equation

An example for a custom 1D model.
"""

# ╔═╡ 3d04a560-17d8-4b73-a142-7952019765a9
md"""
We set up the GPE:
```math
...
# ╔═╡ fbc2b442-eab5-41f1-aabc-ba5e1e0e6633
@bind Ecut_gpe Slider(400:100:1000; show_value=true, default=500)

# ╔═╡ 361860a0-496c-47a6-97bf-22f4804f6a83
begin
	a = 10
	lattice = a .* [[1 0 0.]; [0 0 0]; [0 0 0]]

	V(x) = (x - a/2)^2  # Confining potential

	terms = [Kinetic(),
	         ExternalFromReal(r -> V(r[1])),
	         LocalNonlinearity(ρ -> ρ^2),
	]
	model_gpe = Model(lattice; n_electrons=1, terms, spin_polarization=:spinless)
	basis_gpe = PlaneWaveBasis(model_gpe, Ecut=Ecut_gpe, kgrid=(1, 1, 1))
end;

# ╔═╡ 67ceb38e-2418-45e4-a638-16f737f4c3ea
begin
	scfres = direct_minimization(basis_gpe; tol=1e-8) # This is a constrained preconditioned LBFGS
	scfres.energies
end

# ╔═╡ d18573e8-369a-4ba8-9a8b-893962bfa822
md"""
...
# ╔═╡ 4fb24ba8-48ef-4222-9e67-8831ec8735e3
begin
	ρ = real(scfres.ρ)[:, 1, 1, 1]  # converged density, first spin component
	ψ_fourier = scfres.ψ[1][:, 1]  # first k-point, all G components, first eigenvector
end;

# ╔═╡ a1dbdd59-28ae-4cc3-bbf6-b447d909314e
md"""Transform the wave function to real space and fix the phase:"""

# ╔═╡ 690a78f6-90b4-490a-a6b2-211ec0931bf7
begin
	ψ = ifft(basis_gpe, basis_gpe.kpoints[1], ψ_fourier)[:, 1, 1]
	ψ /= (ψ[div(end, 2)] / abs(ψ[div(end, 2)]))
end;

# ╔═╡ 63e7044c-0aec-4aff-8ced-a4bcd57e799a
md"""Check whether $ψ$ is normalised:"""

# ╔═╡ 2374744b-c7fb-4123-8fca-c03223078ea0
begin
	x = a * vec(first.(DFTK.r_vectors(basis_gpe)))
	N = length(x)
	dx = a / N  # real-space grid spacing
	sum(abs2.(ψ)) * dx
end

# ╔═╡ b3c5c071-c46a-40bd-99e7-7a6084a29470
begin
	p = plot(x, real.(ψ), label="real(ψ)", lw=2)
	plot!(p, x, imag.(ψ), label="imag(ψ)", lw=2, ls=:dot)
	plot!(p, x, ρ, label="ρ", lw=2)
	plot!(p, x, x -> V(x) / 25, label="V / 25", lw=2, ls=:dash)
end
...
manifest_format = "2.0"
project_hash = "97b815292bd9849a59066d27d6d4543e1f2dd1f7"
