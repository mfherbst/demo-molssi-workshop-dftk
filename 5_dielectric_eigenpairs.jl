...
We want to understand SCF convergence. For simplicity we select a system consisting of $n_repeat_aluminium layers of **aluminium** (a prottypical metal).

"""

# ╔═╡ 7434967b-b988-447a-b346-cac2b4c11776
begin
	n_supercell = floor(Int, n_repeat_aluminium/2)  # Cubic aluminium has 2 layers per cell

	system_Al = attach_psp(bulk(:Al; cubic=true) * (n_supercell, 1, 1);
...
md"""Based on this discretisation we run the SCF
```math
	\rho_{n+1} = \rho_n + \texttt{damping} * P^{-1} \Big[ \rho \Big( V(\rho_n) \Big) - \rho_n \Big].
```
using three different preconditioners, namely
- `SimpleMixing`, i.e. no preconditioner ($P = I$)
- `KerkerMixing` a standard preconditioner for metals
- Our proposed `LdosMixing`.
"""
...
To understand the difference we now look at the eigenpairs of the Jacobian governing the convergence, that is
```math
...
scfres_Al_small = let
	system_Al = attach_psp(bulk(:Al; cubic=true) * (8, 1, 1);
					       Al=artifact"pd_nc_sr_pbe_standard_0.4.1_upf/Al.upf")
	model_Al = model_PBE(system_Al; temperature=1e-3, symmetries=false)
	basis_Al = PlaneWaveBasis(model_Al; Ecut=7, kgrid=(1, 1, 1))
	self_consistent_field(basis_Al; mixing=KerkerMixing(), tol=1e-8, callback=identity)
end

# ╔═╡ a78e34b8-0ab9-4400-a437-ccb9648535d3
begin
	# Function, which applies P^{-1} for the case of KerkerMixing
	Pinv_Kerker(δρ) = DFTK.mix_density(KerkerMixing(), scfres_Al_small.basis, δρ)

	# Function which applies ε† = 1 - χ0 K
    function epsilon(δρ)
        δV   = apply_kernel(scfres_Al_small.basis, δρ; ρ=scfres_Al_small.ρ)
        χ0δV = apply_χ0(scfres_Al_small, δV)
        δρ - χ0δV   
    end
end;

# ╔═╡ a70d053e-4257-4458-86bf-aef38557493b
md"""
With these functions available we can now compute the desired eigenvalues. For simplicity we only consider the first few largest ones.
"""

# ╔═╡ a6c74c32-9329-4d11-8b05-9effa23a108a
λ_Simple, X_Simple = eigsolve(epsilon, randn(size(scfres_Al_small.ρ)), 3, :LM;
                              tol=1e-3, eager=true, verbosity=2)

# ╔═╡ a513d7cd-2431-4275-8965-286510668621
md"We can also e.g. plot the largest eigenmode."

# ╔═╡ a5cb6eac-57a8-45fe-8436-7ab7ea8d8f09
let
...
# ╔═╡ 82399cad-6e2e-4586-8a0c-40a9d420c463
md"This mode can be physically interprested as the reason why this SCF converges slowly. For example in this case it displays a displacement of electron density from the centre to the extremal parts of the unit cell. This phaenomenon is called charge-sloshing."

# ╔═╡ 7fb6f4bf-3ccb-44be-8a69-735cb004e48d
md"We repeat the exercise for the Kerker-preconditioned dielectric operator:"

# ╔═╡ 114d7d12-e317-4f1e-86bf-d75d8ab3cc23
λ_Kerker, X_Kerker = eigsolve(Pinv_Kerker ∘ epsilon,
							  randn(size(scfres_Al_small.ρ)), 3, :LM;
							  tol=1e-3, eager=true, verbosity=2)

# ╔═╡ e297eedc-d45e-466c-834e-e59cf2814de3
let
...
# ╔═╡ af492cf0-8bdb-4cae-91c8-b189c969f8df
md"""
Clearly the charge-sloshing mode is no longer dominating.

Overview of largest eigenvalues:

- `SimpleMixing`: $(maximum(real.(λ_Simple)))
- `KerkerMixing`: $(maximum(real.(λ_Kerker)))

Since the smallest eigenvalue is around $1$ in both cases, this implies that the conditioning improves noticably when `KerkerMixing` is used.
...
git-tree-sha1 = "67eb567ceaf976505f1badc800a8d089ffbdb493"
